...
HARD TESTS — every pick must pass ALL: (1) the "ALREADY KNOWS" test — global luxury mega-flagships (Louis Vuitton, Gucci), department stores (Holt Renfrew, Selfridges) and mall anchors FAIL; an industry visitor already knows them (exception: a location that is itself a destination experience, like Dover Street Market — say why in the take). (2) GEOGRAPHY — the store must have a real, currently-operating location in the requested neighborhood/city; if unsure, drop it. (3) INSIDER VALUE — would a plugged-in local be impressed or bored? Bored = cut. (4) NO MID FILLER — minor basics labels and ordinary consignment don't qualify.`;
